...
<details>
  <summary>챌린지 결과 및 피드백</summary>

- 수치상(데이터상) 나름 만족
  - 프로그래머스 랭킹 상승(약13000 → 약4000위)
  - 문제 풀이 수 (기록상 85문제) : 목표한 것을 다 풀지는 못했지만 **약 70퍼 이상** 완료에 만족한다.
- 이번 챌린지의 목표는 코테 준비 및 문제 유형이 익숙해지는 것이였다. 이 부분은 어느정도 성과를 이룬 것 같다.
- `자료구조나 알고리즘에 대한 개념적인 부분` : 문제 풀면서 찾아본 부분이라 따로 정리를 해봐야할 필요성을 느꼈다. 그리고 모든 자료구조에 대해선 구현을 최소 한번씩은 구현을 해볼것!
- `문제 해결 능력 향상` : 문제를 많이 풀면서 어느정도 성장하고 있음을 느낄때가 있었다.
- `문제를 푸는 것 이상의 인사이트를 얻었는가` : 내가 남긴 기록들이 인사이트의 증거일까? 아직은 물음표.
